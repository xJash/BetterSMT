...
public class BetterSMT : BaseUnityPlugin {
    ...
    private void Awake() {
        // === !Auto Save Settings! ===
        AutoSaveEnabled = Config.Bind("Save Settings","Enables Auto Saving",false,new ConfigDescription("Enables or disables automatic saving"));
        AutoSaveTimer = Config.Bind("Save Settings","Amount of time between saves",120,new ConfigDescription("Adjusts the amount of time between auto saves in seconds, default is 120seconds or 2minutes",new AcceptableValueRange<int>(30,900)));
        AutoSaveDuringDay = Config.Bind("Save Settings","Enables Auto Saving during the day",false,new ConfigDescription("Enables or disables saving while the store is open, default only autosaves while closed"));
        ...
        // === !Low Stock Alerts! ===
        LowStockAlertEnabled = Config.Bind("Low Stock Alerts","Enable Low Stock Alerts",false,new ConfigDescription("Show alerts when products drop below the threshold"));
        LowStockThreshold = Config.Bind("Low Stock Alerts","Low Stock Threshold",2,new ConfigDescription("The stock count at which alerts are triggered",new AcceptableValueRange<int>(1,1000)));

        // === !Employee & Customer Settings! ===
        EmployeExtraCheckoutMoney = Config.Bind("Employee & Customer Settings","Employe Increased Income While Checking Customer Out Perk",0.1f,new ConfigDescription("Adjust the amount of extra income you receive when an employee checks out a customer (Higher = more income)",new AcceptableValueRange<float>(0f,0.3f)));
        EmployeeCheckoutPerPerk = Config.Bind("Employee & Customer Settings","Employee Checkout Time Reduction Perk 1",.15f,new ConfigDescription("Adjust the amount of time employees wait to scan items in checkout (Perk 1) (Lower = slower)",new AcceptableValueRange<float>(0.01f,0.24f)));
        MaxCustomerCart = Config.Bind("Employee & Customer Settings","Maximum amount of product customers will buy",25,new ConfigDescription("Adjust the maximum amount of product customers will buy",new AcceptableValueRange<int>(1,75)));
        BaseCustomerCart = Config.Bind("Employee & Customer Settings","Minimum amount of product customers will buy",5,new ConfigDescription("Adjust the minimum amount of product customers will buy",new AcceptableValueRange<int>(1,15)));
        BaseCustomerSpawns = Config.Bind("Employee & Customer Settings","Minimum amount of customers that will spawn",3,new ConfigDescription("Adjust the minimum amount of customer's that can spawn",new AcceptableValueRange<int>(1,9)));
        MaxCustomerInStore = Config.Bind("Employee & Customer Settings","Max amount of customers in store",70,new ConfigDescription("Adjust the amount of customers that can spawn at one time",new AcceptableValueRange<int>(1,210)));
        CustomersPerPerk = Config.Bind("Employee & Customer Settings","Extra Customers per perk",1,new ConfigDescription("Adjust the amount of customers you gain per perk (Higher number = more customers)",new AcceptableValueRange<int>(1,5)));
        EmployeesLevel = Config.Bind("Employee & Customer Settings","Employees Level",0,new ConfigDescription("Adjust the level of employee's that spawn (1 sets all of their stats to minimum, 30 sets them all to max)",new AcceptableValueRange<int>(0,30)));
        EmployeeSpeedPerPerk = Config.Bind("Employee & Customer Settings","Employee Speed Per Perk",.2f,new ConfigDescription("Adjust the amount of speed employees gain per perk (Higher = faster)",new AcceptableValueRange<float>(.01f,.6f)));
        EmployeeRestockPerPerk = Config.Bind("Employee & Customer Settings","Employee Restock Time Reduction Per Perk",0.05f,new ConfigDescription("Adjust the amount of time it takes for employees to restock per perk (Lower = faster)",new AcceptableValueRange<float>(0.01f,0.15f)));

        // === !Thieves & Crime Settings! ===
        AllNPCAreThieves = Config.Bind("Thieves & Crime Settings","All Thieves",false,new ConfigDescription("Causes every NPC to be a thief"));
        SelfCheckoutTheft = Config.Bind("Thieves & Crime Settings","Self-Checkout Theft",true,new ConfigDescription("Enables or disables default game's theft from self checkout"));
        DisableAllThieves = Config.Bind("Thieves & Crime Settings","Disable Thieves",false,new ConfigDescription("Prevent thieves from spawning (Does not stop checkout-related thieves)"));
        OneHitThief = Config.Bind("Thieves & Crime Settings","Thiefs Drop Everything On Hit",false,new ConfigDescription("Thiefs Drop Everything On Hit"));

        // === !Gameplay Settings! ===
        InstantSelfCheckout = Config.Bind("Gameplay Setting","Faster Self-Checkout",false,new ConfigDescription("Customers place all items instantly on to the self-checkout"));
        FasterCheckout = Config.Bind("Gameplay Settings","Faster Checkout",false,new ConfigDescription("Customers place all items instantly on to the checkout"));
        ShowFPS = Config.Bind("Gameplay Settings","Show FPS Counter",false);
        ShowPing = Config.Bind("Gameplay Settings","Show Ping Counter",false);
        DisableAllTrash = Config.Bind("Gameplay Settings","Despawns trash",false,new ConfigDescription("Despawns all trash at the end of the day"));
        AlwaysAbleToDeleteMode = Config.Bind("Gameplay Settings","Always access to delete",false,new ConfigDescription("Delete shelves and others while customers are in store and store is open"));
        ...
        AutoPickupRange = Config.Bind("Mini Transport Vehicle","Change the range of auto-picking up boxes",3f,new ConfigDescription("Adjusts how far away a box will be automatically picked up onto the Mini Transport Vehicle.",new AcceptableValueRange<float>(1f,30f)));
        MaxBoxes = Config.Bind("Mini Transport Vehicle","Change the max boxes",6,new ConfigDescription("Adjusts the max amount of boxes the Mini Transport Vehicle will hold at one time.",new AcceptableValueRange<int>(6,16)));
        ...
        DropCooldown = Config.Bind("Mini Transport Vehicle","Item Pickup Cooldown",10f,new ConfigDescription("Changes how long it will wait to pickup boxes again after the drop hotkey has been pressed.",new AcceptableValueRange<float>(1f,30f)));


        // === !Random Features! ===
        ExpRate = Config.Bind("Random Features","EXP Rate",1,new ConfigDescription("Adjusts how much EXP you earn per transaction.",new AcceptableValueRange<int>(1,10)));
        ...
        AutoAdjustPriceDailyValue = Config.Bind("Pricing Assistance","Adjust the amount prices are automatically set to every day",2f,new ConfigDescription("Adjusts the amount prices are set to be multiplied by daily. Value of 2x is 2$ * 2 = 4$. Value of 1.99x is 2$*1.99=3.98",new AcceptableValueRange<float>(1f,2f)));
        ...
        DoublePrice = Config.Bind("Double Price Gun","Enable Double Price module",new KeyboardShortcut(KeyCode.Q),new ConfigDescription("Hotkey to enable and disable double price module"));
        ...
        RoundDownSwitch = Config.Bind("Double Price Gun","Round Down Shortcuts",new KeyboardShortcut(KeyCode.Q,KeyCode.LeftControl),new ConfigDescription("Hotkey to round down the double price"));
        ...
        SelfCheckoutLimit = Config.Bind("Random QoL","Product limit on self checkout",4,new ConfigDescription("Limits the amount of item's a customer can have before using the self checkout",new AcceptableValueRange<int>(0,100)));
        MaxBoxSize = Config.Bind("Random QoL","Modify amount of products in boxes",1,new ConfigDescription("** WARNING THIS IS EXTREMELY BUGGY IN MULTIPLAYER ** Multiples the amount of product in a box, aswell as it's cost. Higher = more products in box and higher cost https://imgur.com/a/QT5l2Ky",new AcceptableValueRange<int>(1,50)));
        AutoPayAllInvoices = Config.Bind("Debt","Enables or disables automatic invoice payment",false,new ConfigDescription("Enables or disables paying for invoices automatically"));
